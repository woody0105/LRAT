...
if [ ! -e "$HOME/ffmpeg/libavcodec/libavcodec.a" ]; then
  ...
  ./configure --prefix="$HOME/compiled" --enable-libx264 --enable-gnutls --enable-gpl --enable-libass --enable-static --enable-cuda --enable-cuvid --enable-nvenc --enable-decoder=h264_cuvid 
  make
  make install
fi